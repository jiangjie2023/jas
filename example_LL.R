...
for(i in 1:burn_in)
{ 
  lambda<-(rgamma(n,shape=2,rate=2))^(-1)
  
  logy=Cens*log(C)+(1-Cens)*rtruncnorm(n=n,a=log(C),b=Inf,mean=mu0,sd=sigma0/sqrt(lambda))
  y<-exp(logy)
  ...
  a0= -1/sigma0*sum(abs(log(y)-propmu))-(-1/sigma0*sum(abs(log(y)-mu0)))
  ...
  a1=(-n-a)*props-1/props*sum(abs(log(y)-mu0))-
    ((-n-a)*sigma0-1/sigma0*sum(abs(log(y)-mu0)))
  if(a1< -100){a1=-100}
...
DIC=DIC.LLAP(y,Cens,MCMCsamples)
