...
burn_in=100000
mu1=c(0)
burn=matrix(0,2,burn_in)
for(i in 1:burn_in)
{ 
  y=y
  for(i1 in 1:9)
 {logy0=rtrunc(1,spec="norm",a=log(C[cen_ind[i1]]),b=Inf,mean=mu0,sd=sigma0)
 
  y[cen_ind[i1]]<-exp(logy0)}
  
  
  propmu=rnorm(1,mu0,stepsize[1])
  mu1[i]=propmu
  #a0=exp(-1/2/sigma0^2*sum((log(y)-propmu)^2))/exp(-1/sigma0^2/2*sum(abs(log(y)-mu0)^2))
  a0=-1/2/sigma0^2*sum((log(y)-propmu)^2)-(-1/sigma0^2/2*sum(abs(log(y)-mu0)^2))
  
  if(a0< -100){a0=-100}
  if(runif(1)<exp(a0)){mu0=propmu}else{mu0=mu0}
  burn[1,i]=mu0
  props=rtruncnorm(1,0,Inf,sigma0,stepsize[2])
  a1=(-n-a)*props-1/2/props^2*sum(abs(log(y)-mu0)^2)-
    ((-n-a)*sigma0-1/2/sigma0^2*sum(abs(log(y)-mu0)^2))
  if(a1< -100){a1=-100}
